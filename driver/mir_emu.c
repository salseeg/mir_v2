...
static int mir_ioctl (struct inode *none, struct file *none2, unsigned int x, unsigned long y)
{
#ifdef DEBUG
		...
		/* фотографируем порты*/
		/*printk("You send %d", x); //  debug info   */
		ph.inner = (unsigned char) x;	 
		ph.outer = (unsigned char) (x>>8);
